...
num_pts = np.array([ 0.13267733 , 0.81788035,  0.0928159 ],dtype=np.float32)
...
class myUnet(object):
	...
	def train(self):
		...
		mcb = My_Callback(x_val,y_val,self.model,self.save_file)
		prev_val_acc = 0
...
class My_Callback(callbacks.Callback):
    def __init__(self,x_val, y_val,network,fname):
    	self.X_val = x_val
    	self.Y_val = y_val
    	self.num_epochs = 0
    	self.calc_epoch = 5
